...
from crown.core.key import CPubKey
from crown.core.serialize import ImmutableSerializable
from crown.wallet import P2PKHBitcoinAddress
import crown
import base64
import sys
...
class BitcoinMessage(ImmutableSerializable):
    ...
    def stream_deserialize(cls, f):
        magic = crown.core.serialize.BytesSerializer.stream_deserialize(f)
        message = crown.core.serialize.BytesSerializer.stream_deserialize(f)
        return cls(message, magic)

    def stream_serialize(self, f):
        crown.core.serialize.BytesSerializer.stream_serialize(self.magic, f)
        crown.core.serialize.BytesSerializer.stream_serialize(self.message, f)
