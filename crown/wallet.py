...
import crown.bech32
import crown.core
import crown.core.key
import crown.core.script as script
...
class CBase58BitcoinAddress(crown.base58.CBase58Data, CBitcoinAddress):
    ...
    def from_bytes(cls, data, nVersion):
        ...
        if nVersion == crown.params.BASE58_PREFIXES['SCRIPT_ADDR']:
            self.__class__ = P2SHBitcoinAddress

        elif nVersion == crown.params.BASE58_PREFIXES['PUBKEY_ADDR']:
...
class P2SHBitcoinAddress(CBase58BitcoinAddress):
    ...
    def from_bytes(cls, data, nVersion=None):
        if nVersion is None:
            nVersion = crown.params.BASE58_PREFIXES['SCRIPT_ADDR']

        elif nVersion != crown.params.BASE58_PREFIXES['SCRIPT_ADDR']:
    ...
    def from_scriptPubKey(cls, scriptPubKey):
        ...
        if scriptPubKey.is_p2sh():
            return cls.from_bytes(scriptPubKey[2:22], crown.params.BASE58_PREFIXES['SCRIPT_ADDR'])
    ...
    def to_scriptPubKey(self):
        ...
        assert self.nVersion == crown.params.BASE58_PREFIXES['SCRIPT_ADDR']
...
class P2PKHBitcoinAddress(CBase58BitcoinAddress):
    ...
    def from_bytes(cls, data, nVersion=None):
        if nVersion is None:
            nVersion = crown.params.BASE58_PREFIXES['PUBKEY_ADDR']
    ...
    def from_pubkey(cls, pubkey, accept_invalid=False):
        ...
        if not accept_invalid:
            if not isinstance(pubkey, crown.core.key.CPubKey):
                pubkey = crown.core.key.CPubKey(pubkey)
            if not pubkey.is_fullyvalid:
                raise CBitcoinAddressError('invalid pubkey')

        pubkey_hash = crown.core.Hash160(pubkey)
    ...
    def from_scriptPubKey(cls, scriptPubKey, accept_non_canonical_pushdata=True, accept_bare_checksig=True):
        ...
        if accept_non_canonical_pushdata:
            ...
            except crown.core.script.CScriptInvalidError:
                raise CBitcoinAddressError('not a P2PKH scriptPubKey: script is invalid')

        if scriptPubKey.is_witness_v0_keyhash():
            return cls.from_bytes(scriptPubKey[2:22], crown.params.BASE58_PREFIXES['PUBKEY_ADDR'])
        elif scriptPubKey.is_witness_v0_nested_keyhash():
            return cls.from_bytes(scriptPubKey[3:23], crown.params.BASE58_PREFIXES['PUBKEY_ADDR'])
    ...
    def to_scriptPubKey(self, nested=False):
        """Convert an address to a scriptPubKey"""
        assert self.nVersion == crown.params.BASE58_PREFIXES['PUBKEY_ADDR']
...
class CKey(object):
    ...
    def __init__(self, secret, compressed=True):
        self._cec_key = crown.core.key.CECKey()
        self._cec_key.set_secretbytes(secret)
        self._cec_key.set_compressed(compressed)

        self.pub = crown.core.key.CPubKey(self._cec_key.get_pubkey(), self._cec_key)
...
class CBitcoinSecretError(crown.base58.Base58Error):
    pass

class CBitcoinSecret(crown.base58.CBase58Data, CKey):
    ...
    def from_secret_bytes(cls, secret, compressed=True):
        ...
        self = cls.from_bytes(secret + (b'\x01' if compressed else b''),
                              crown.params.BASE58_PREFIXES['SECRET_KEY'])
        self.__init__(None)
        return self

    def __init__(self, s):
        if self.nVersion != crown.params.BASE58_PREFIXES['SECRET_KEY']:
            raise CBitcoinSecretError('Not a base58-encoded secret key: got nVersion=%d; expected nVersion=%d' % \
                                      (self.nVersion, crown.params.BASE58_PREFIXES['SECRET_KEY']))
